...
| **Training sync (optional)** | In Training mode, PIN-gated live sync posts step progress and results to a shared endpoint (Google Sheets or SharePoint), updating the same row by session key |
